...
public class TrainService {
    ...
    public void addTrain(Train newTrain ) {
        // check if any train with the same trainId exists
        Optional<Train> existingTrain = trainList.stream()
                .filter(train -> train.getTrainId().equalsIgnoreCase(newTrain.getTrainId()))
                .findFirst();
        if (existingTrain.isPresent()) {
            //If a train with the same trainID exists then update it instead of adding a new one
            updateTrain(newTrain);

        } else {
            //otherwise update the new train to the trainlist
    ...
    private void saveTrainListToFile() {
        try {
            ...
            e.printStackTrace();  // Handles the excptions based on the requirements of your application
